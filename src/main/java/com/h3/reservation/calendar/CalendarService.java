...
public class CalendarService {
     ...
     */
    public Event insertEvent(final ReservationDateTime fetchingDate, final CalendarId calendarId, String room, String attendee, String description) throws IOException {
    ...
    private void checkValidReservation(final ReservationDateTime fetchingDate, final CalendarId calendarId, String room) {
        List<Event> eventsByTime = findReservation(fetchingDate, calendarId);
        List<Event> eventsByRoom = eventsByTime.stream()
                .filter(e -> e.getSummary().contains(room))
                .collect(Collectors.toList());

        if (eventsByRoom.size() > 0) {
